...
const index = (req, res, next) => {
    res.status(200).json({
        'message': 'The API is up and running on port 5000',
    ...
    var check = password_resets.findOne({
        ...
        password_resets.update({
            ...
            if (msg === null) {
                ...
                res.status(200).json({
                    'message': 'Verification token has been re-sent to your email!',
                    'status': 1
                });
            }
        });
...
async function register(req, res) {
    try {
        ...
        const saveUser = await insertData(Users, {
        ...
        if (bankDetails.status == 'success' && bankDetails.data.response_code == '02') {
            await insertData(bank_acct, {
                accountName: qry.firstName + ' ' + qry.lastName,
        ...
        res.status(201).json({
            'message': 'User ' + qry.firstName + ' ' + qry.lastName + ' ' + 'created sucessfully!',
...
async function login(req, res) {
    try {
        ...
        res.status(500).json({ 'message': 'Error logging in, username doesnt exist', 'status': 0 });
    }
    ;
}
